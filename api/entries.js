...
async function open(b) {
  ...
  if (c.status !== 'running')
    throw new HttpError(
      403,
      c.status === 'scheduled' ? 'Les ajouts commencent le 27 septembre.' : 'Le défi est terminé.',
    );
  if (typeof b.participantId !== 'string' || !(await readerExists(b.participantId)))
    throw new HttpError(404, 'Ce lecteur est introuvable.');
  return { now, c };
}
