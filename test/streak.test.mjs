...
assert.equal(new Date(s.expiresAt).toISOString(), '2026-09-29T22:00:00.000Z');
assert.equal(streakFor(e, at('2026-09-28T16:59:00Z')).atRisk, false, 'Mon 18:59: under 18 h since the reading');
assert.equal(streakFor(e, at('2026-09-28T17:01:00Z')).atRisk, true, 'Mon 19:01: hourglass on (6 h before the 24 h mark)');
...
const withDeleted = read('2026-09-27T08:00:00Z', '2026-09-27T23:00:00Z'); withDeleted[1].deletedAt = 'x';
