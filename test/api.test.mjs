...
function run([c, ...a]) {
  switch (c) {
    ...
    case 'HEXISTS': return hash(a[0]).has(a[1]) ? 1 : 0;
    case 'EVAL': { const [, , names, readers, key, id, json, max] = a;
      if (hash(readers).size >= Number(max)) return -1;
      if (hash(names).has(key)) return 0;
      hash(names).set(key, id); hash(readers).set(id, json); return 1; }
  }
...
async function call(name, method, {body, query = {}, header = true} = {}) {
  ...
  let out = {};
  const res = {statusCode: 0, setHeader() {}, end(s) { out = JSON.parse(s); }};
...
assert.equal(s.status, 200); assert.equal(s.challenge.status, 'scheduled'); assert.equal(s.challenge.daysUntilStart, 4);
assert.equal(s.readerCount, 0); assert.equal(s.totalPages, 0);
const camille = await call('participants', 'POST', {body: {name: '  Camille  ', goal: 300}});
assert.equal(camille.status, 200); assert.equal(camille.participant.name, 'Camille');
assert.equal((await call('participants', 'POST', {body: {name: 'camille', goal: 100}})).status, 409, 'duplicate name, case-insensitive');
assert.equal((await call('participants', 'POST', {body: {name: 'Camillé', goal: 100}})).status, 409, 'duplicate name, accent-insensitive');
...
assert.equal((await call('participants', 'POST', {body: {name: 'Léa', goal: 100}, header: false})).status, 403, 'missing header');
...
assert.equal((await call('entries', 'POST', {body: {participantId: cid, pages: 12, requestId: req1}})).status, 200, 'retry is idempotent');
assert.equal((await call('entries', 'POST', {body: {participantId: cid, pages: 13, requestId: req1}})).status, 409, 'reused request id');
assert.equal((await call('entries', 'POST', {body: {participantId: cid, pages: 0, requestId: uuid()}})).status, 400);
assert.equal((await call('entries', 'POST', {body: {participantId: cid, pages: '5', requestId: uuid()}})).status, 400);
assert.equal((await call('entries', 'POST', {body: {participantId: 'nope', pages: 5, requestId: uuid()}})).status, 404);
...
assert.equal(p.participant.pages, 42); assert.equal(p.participant.streak, 2); assert.equal(p.participant.entries.length, 2);
...
assert.equal((await call('entries', 'DELETE', {body: {participantId: other.participant.id, entryId: req2}})).status, 404, 'cannot cancel for someone else');
assert.equal((await call('entries', 'DELETE', {body: {participantId: cid, entryId: req2}})).status, 200);
assert.equal((await call('entries', 'DELETE', {body: {participantId: cid, entryId: req2}})).status, 200, 'double cancel is harmless');
p = await call('profile', 'GET', {query: {id: cid}});
assert.equal(p.participant.pages, 12); assert.equal(p.participant.entries.length, 1);
s = await call('state', 'GET');
assert.equal(s.totalPages, 12); assert.equal(s.readerCount, 2);
...
assert.equal((await call('entries', 'POST', {body: {participantId: cid, pages: 5, requestId: uuid()}})).status, 403);
assert.equal((await call('participants', 'POST', {body: {name: 'Zoé', goal: 100}})).status, 403);
s = await call('state', 'GET');
assert.equal(s.challenge.status, 'finished'); assert.equal(s.totalPages, 12);
